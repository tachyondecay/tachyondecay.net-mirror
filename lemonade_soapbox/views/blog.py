...
@bp.route('/')
def index():
    articles = Article.published().order_by(Article.date_published.desc()).limit(10).all()
    return render_template('blog/views/article_list.html',
                           articles=articles,
...
def show_draft(handle):
    ...
    return render_template('blog/views/single_article.html',
                           article=article)
...
@bp.route('/feed/')
def default_feed():
    return redirect(url_for('.show_feed', format=current_app.config['DEFAULT_FEED_FORMAT']))
...
def show_feed(format):
    articles = Article.published().order_by(Article.date_published.desc()).limit(10).all()
    return Response(
        render_template('blog/articles/' + format + '.xml',
                        articles=articles,
...
def all_tags():
    ...
    return render_template('blog/views/tags.html',
...
def show_tag(handle):
    ...
    page_title = ngettext('%(num)d Article Tagged with “%(t)s”',
                          '%(num)d Articles Tagged with “%(t)s”',
                          len(articles)) % {'num': len(articles),
                                            't': tag.label}
...
def year_archive(year):
    ...
    if not articles:
        abort(404)
    page_title = ngettext('%(num)d Article from %(year)s',
                          '%(num)d Articles from %(year)s',
                          len(articles)) % {'num': len(articles), 'year': year}

    return render_template('blog/views/year_archive.html',
                           articles=articles,
                           page_title=page_title,
                           year=year)
...
def month_archive(year, month):
    ...
    if not articles:
        abort(404)
    page_title = ngettext('%(num)d Article from %(month)s %(year)s',
